...
計算 Speed-up (數字大於 1 表示 Column 顯示的方法優於 Row 顯示的方法)

```{r tspdup, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE}
spdv <- c(t1, t2, t3, t4); names(spdv) <- sprintf("Method %d", 1:4)
spdup <- spdv %*% t(1/spdv)
dimnames(spdup) <- list(sprintf("Method %d", 1:4), sprintf("Method %d", 1:4))
cat("CPU times (seconds):")
print(spdv)
cat("Speed-up:")
print(round(spdup, 3))
```
